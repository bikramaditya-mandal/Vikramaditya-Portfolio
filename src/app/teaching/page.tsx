...
export default function Teaching() {
  ...
  return (
    <>
      ...
      <div className="pt-36 pb-16 border-b border-white/[0.08] max-w-[1200px] mx-auto px-16 max-md:px-6">
        <FadeUp>
          ...
          <h1 className="font-sans text-[clamp(2.5rem,5vw,4rem)] font-extrabold text-white leading-[1.05] tracking-[-0.03em] mb-3">
            Educating the <span className="text-accent">Next Generation</span>
          </h1>
                  ...
                  <div className={`overflow-hidden transition-all duration-500 ${isOpen ? 'max-h-[4000px]' : 'max-h-0'}`}>
                    <div className="px-16 pb-10 space-y-8 max-md:px-6">
                      {topic.blocks.map((block, bi) => (
                        <div key={bi}>
                          <h4 className="font-sans text-[1rem] font-bold text-cream mb-3 tracking-[-0.01em]">{block.title}</h4>
                          {block.intro && (
                            <p className="text-muted text-[0.88rem] font-light leading-[1.8] mb-2">{block.intro}</p>
                          )}
                          <ul className="space-y-2">
                            {block.items.map((item, ii) => (
                              <li key={ii} className="relative pl-5 text-muted text-[0.88rem] font-light leading-[1.8]">
                                <span className="absolute left-0 top-[0.15rem] text-accent text-[0.75rem]">—</span>
                                {item}
                              </li>
                            ))}
                          </ul>
      ...
      {/* Philosophy */}
      <FadeUp>
        <div className="bg-[#141414] border-t border-b border-white/[0.08] py-20 px-16 max-md:px-6">
          <div className="max-w-[800px] mx-auto">
            <span className="section-label">Philosophy</span>
            <blockquote className="font-sans text-[clamp(1.3rem,2.5vw,1.8rem)] font-bold text-cream leading-[1.4] tracking-[-0.02em] border-l-[3px] border-accent pl-8 my-6">
            ...
            <p className="text-muted text-[0.95rem] font-light leading-[1.9]">
