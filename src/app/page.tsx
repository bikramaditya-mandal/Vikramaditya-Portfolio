...
export default function Home() {
  return (
    <>
      ...
      {/* HERO */}
      <section className="relative min-h-screen bg-[#1c1c1c] grid grid-cols-[65%_35%] max-md:grid-cols-1 items-center overflow-hidden">
        <div className="relative z-10 px-[8vw] md:px-[10vw] pt-32 pb-20">
          <p className="font-sans text-base font-light text-white tracking-wide mb-5 animate-[fadeUp_0.7s_ease_both]">Hi there!</p>
          <h1 className="font-sans text-[clamp(2rem,3.2vw,3.4rem)] font-extrabold leading-[1.1] tracking-[-0.03em] text-white mb-6 animate-[fadeUp_0.7s_0.1s_ease_both]">
            I'm Bikramaditya Mandal,<br />
            a <span className="text-white">Quantum Physicist</span><br />
            decoding molecular collisions
          </h1>
          <p className="font-sans text-[0.95rem] font-light text-white/80 max-w-[400px] leading-[1.85] mb-10 animate-[fadeUp_0.7s_0.2s_ease_both]">
          ...
          <Link href="#about" className="inline-block px-8 py-3 border-[1.5px] border-white/30 text-white font-sans text-xs font-semibold tracking-[0.14em] uppercase rounded-full transition-all hover:bg-white hover:text-[#1c1c1c] animate-[fadeUp_0.7s_0.3s_ease_both]">
            MORE ABOUT ME
          </Link>
        </div>

        {/* Photo */}
        <div className="relative h-[90vh] -ml-[22%] w-[115%] overflow-hidden max-md:absolute max-md:inset-0 max-md:ml-0 max-md:w-full max-md:h-full max-md:opacity-25">
          <Image src="/assets/bikramaditya-mandal.png"
            alt="Dr. Bikramaditya Mandal" fill
            className="object-cover object-top grayscale contrast-100 brightness-90"
            priority sizes="45vw" />
          {/* Left fade */}
          <div className="absolute inset-y-0 left-0 w-[60%] bg-gradient-to-r from-[#1c1c1c] to-transparent z-10 pointer-events-none max-md:hidden" />
          {/* Bottom fade */}
          <div className="absolute bottom-0 left-0 right-0 h-[25%] bg-gradient-to-t from-[#1c1c1c] to-transparent z-10 pointer-events-none" />
        </div>

        {/* Scroll hint */}
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-20 col-span-2">
          <div className="w-6 h-10 border-2 border-white/25 rounded-full flex justify-center pt-1.5">
            <div className="w-1 h-2 bg-white/45 rounded scroll-wheel-animate" />
          </div>
        </div>
      </section>

      {/* ABOUT */}
      <section className="bg-[#f8f7f4] py-28 px-[5vw]" id="about">
        <div className="max-w-[1200px] mx-auto grid grid-cols-[1fr_1.15fr] gap-24 items-center max-lg:grid-cols-1 max-lg:gap-10">
          <FadeUp>
            <div className="relative w-full aspect-[4/5] max-lg:aspect-[3/2] max-lg:max-h-[360px] overflow-hidden rounded">
          ...
          <FadeUp delay={100}>
            <h2 className="font-sans text-[clamp(1.8rem,3vw,2.6rem)] font-extrabold text-[#111] leading-[1.1] tracking-[-0.03em] mb-7">
      ...
      {/* STATS */}
      <div className="bg-[#111] border-y border-white/[0.06] py-10 px-[5vw]">
        <div className="max-w-[1200px] mx-auto flex items-center justify-center flex-wrap">
          {stats.map((s, i) => (
            <div key={s.l} className="flex items-center">
              <FadeUp delay={i * 80} className="flex flex-col items-center gap-1 px-12 py-2">
                <span className="font-sans text-[2.2rem] font-extrabold tracking-[-0.03em] text-white leading-none">{s.n}</span>
                <span className="font-mono text-[0.65rem] tracking-[0.15em] uppercase text-white/35">{s.l}</span>
              </FadeUp>
      ...
      {/* RESEARCH FOCUS */}
      <section className="bg-[#0e0e0e] py-28 px-[5vw]">
        <div className="max-w-[1200px] mx-auto">
          <div className="mb-14">
            <span className="eyebrow font-mono">Research Focus</span>
            <h2 className="font-sans text-[clamp(2rem,4vw,3rem)] font-extrabold text-white leading-[1.1] tracking-[-0.03em]">
              Where Physics Meets<br /><span className="text-white">the Frontier</span>
            </h2>
          </div>
          <div className="grid grid-cols-2 gap-px bg-white/[0.07] border border-white/[0.07] rounded-lg overflow-hidden max-md:grid-cols-1">
            {focusCards.map((c, i) => (
              <FadeUp key={c.num} delay={i * 80}>
                <Link href={c.href}
                  className={`group relative flex flex-col gap-3 p-10 h-full transition-colors duration-300
                    ${c.gold ? 'bg-[#141209] hover:bg-[#1a1810]' : 'bg-[#121212] hover:bg-[#181818]'}`}>
                  <div className='text-white'>{c.icon}</div>
                  <span className="absolute top-5 right-7 font-mono text-[0.62rem] text-white/20 tracking-[0.12em]">{c.num}</span>
                  <h3 className="font-sans text-[1.2rem] font-bold text-white leading-[1.2] tracking-[-0.01em] mt-2">{c.title}</h3>
                  <p className="font-sans text-[0.87rem] font-light text-white/45 leading-[1.8] flex-1">{c.body}</p>
                  <span className="font-mono text-[0.72rem] text-accent tracking-[0.08em] transition-all duration-300 group-hover:tracking-[0.14em]">Explore →</span>
                </Link>
              </FadeUp>
            ))}
          </div>
        ...
        <div className="max-w-[900px] mx-auto">
          <div className="mb-10">
            ...
            <h2 className="font-sans text-[clamp(1.8rem,3vw,2.5rem)] font-extrabold text-[#111] leading-[1.1] tracking-[-0.03em]">
              Recent <span className="text-[#888]">Work</span>
            </h2>
          </div>
          <div>
            {pubs.map((p, i) => (
              <FadeUp key={p.href} delay={i * 60}>
                <a href={p.href} target="_blank" rel="noopener noreferrer"
                  ...
                  <span className="font-mono text-[0.7rem] text-accent">{p.year}</span>
                  <div>
          ...
          <div className="mt-10 text-center">
            <Link href="/publications"
              className="inline-block px-10 py-3 border-[1.5px] border-[#222] text-[#222] font-sans text-xs font-semibold tracking-[0.14em] uppercase rounded-full transition-all hover:bg-[#222] hover:text-white">
      ...
      {/* AWARDS */}
      <section className="bg-[#0e0e0e] border-t border-white/[0.06] py-20 px-[5vw]">
        <div className="max-w-[1200px] mx-auto">
          <span className="eyebrow font-mono">Recognition</span>
          <div className="flex items-center flex-wrap mt-8">
            {awards.map((a, i) => (
              <div key={a.year} className="flex items-center">
                <FadeUp delay={i * 80} className="flex-1 min-w-[200px] px-8 first:pl-0">
                  <span className="block font-mono text-[0.68rem] text-accent tracking-[0.15em] mb-2">{a.year}</span>
                  <p className="font-sans text-[0.9rem] font-light text-white/50 leading-[1.6]">{a.text}</p>
                </FadeUp>
                {i < awards.length - 1 && <div className="w-px h-12 bg-white/10 flex-shrink-0" />}
              </div>
            ))}
          </div>
        </div>
      ...
      {/* CTA */}
      <section className="bg-[#f8f7f4] py-28 px-[5vw] text-center border-t border-[#e8e5e0]">
        <div className="max-w-[640px] mx-auto">
          <h2 className="font-sans text-[clamp(2.5rem,5vw,4rem)] font-extrabold text-[#111] leading-[1.0] tracking-[-0.04em] mb-5">
            Let's <span className="text-[#aaa]">Connect</span>
          </h2>
          <p className="font-sans text-[0.95rem] font-light text-[#777] leading-[1.8] mb-10">
